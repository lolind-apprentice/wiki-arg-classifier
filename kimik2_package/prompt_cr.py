...
def build_prompt(train_df, k=K_EXAMPLES, seed=RANDOM_STATE): # seed is so it gets different examples every time
    ...
    * up-sample or down-sample every label to exactly k rows
    ...
    prompt = (
        ...
        "TASK: Classify the sentence into ONE of these labels: {fact, policy, value, other, editorial/meta}"
        "DEFINITIONS:\n"
        ...
        "- editorial/meta: suggestion on improving or maintaining the article, anything else (agreement, off-topic, etc.)\n"
            "INSTRUCTIONS:\n"
